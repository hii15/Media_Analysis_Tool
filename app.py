...
edited_df = st.data_editor(
    ...
    column_config={
        ...
        "CTR(%)": st.column_config.NumberColumn("CTR(%)", disabled=True),
...
if not final_df.empty and final_df['날짜'].notnull().any():
    ...
    # 심화 분석 탭
    t1, t2 = st.tabs(["📉 성과 안정성", "🎯 효율 맵"])
    with t1:
        st.plotly_chart(px.box(final_df, x="매체", y="CTR(%)", color="매체"), use_container_width=True)
    with t2:
        st.plotly_chart(px.scatter(final_df, x="CPM", y="CTR(%)", size="비용", color="소재명", text="소재명"), use_container_width=True)