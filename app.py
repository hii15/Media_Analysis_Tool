...
if not final_df.empty and 'ID' in final_df.columns and len(final_df['ID'].unique()) >= 2:
    ...
    p_list = sorted(final_df['ID'].unique())
    c1, c2 = st.columns(2)
    with c1: item_a = st.selectbox("기준 상품(A)", p_list, index=0)
    with c2: item_b = st.selectbox("비교 대상(B)", p_list, index=1)
    ...
    try:
        ...
        st.warning("선택한 상품의 데이터가 부족하여 분석을 시각화할 수 없습니다.")